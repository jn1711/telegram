...
def get_currency_keyboard():
    """Возвращает клавиатуру с выбором валют"""
    buttons = [
...
@dp.message()
async def convert_amount(message: types.Message):
    user_id = message.from_user.id
    user_info = user_data.get(user_id)
    
    if not user_info:
        await message.answer("Сначала выбери направление конверсии через /start 💱")
        return
    ...
    amount_str = _fmt_num(amount, 2)
    converted_str = _fmt_num(converted, 2)
    
    # Отправляем результат конвертации
    result_text = f"✅ {amount_str} {from_currency} = {converted_str} {to_currency}"
    await message.answer(result_text)
    
    # Отправляем предложение конвертировать ещё (отдельным сообщением)
    keyboard = get_currency_keyboard()
    await message.answer("Конвертировать ещё?", reply_markup=keyboard)
    
    # Очищаем данные пользователя после конвертации
